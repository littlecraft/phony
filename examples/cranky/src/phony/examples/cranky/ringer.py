...
class RingSourceSelector(ClassLogger):
  """
    Controls the RTR314F03 bi-stable relay which selects
    the source signal for the bell ringer (either the
    signal internally generated by the H-Bridge, or directly
    from the old hand-cranked magneto.
  """
  _selected = None
  _outputs = None
  ...
  def select_magneto(self, force = False):
    self._select(1, force)

  def select_internal_driver(self, force = False):
...
class BellRinger(ClassLogger):
  ...
  def __init__(self, io_outputs):
    ...
    self._ring_selector = RingSourceSelector(io_outputs)
    self._ring_selector.select_magneto(force = True)
  ...
  def run(self):
    ...
    self._ring_selector.select_internal_driver()
    ...
    self._ring_selector.select_magneto()
